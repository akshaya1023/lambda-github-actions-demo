...
def _authenticate():
    ...
    auth_data = urllib.parse.urlencode({"username": VAULT_USERNAME, "password": VAULT_PASSWORD}).encode("utf-8")
...
def _get_ha_id_map():
    ...
    _ha_id_map = {}
    try:
        result = _vault_query("SELECT id, name__v FROM health_authority__rim ORDER BY name__v ASC")
        if _is_success(result):
            for r in _all_pages(result):
...
def _get_ha_center_id_map():
    ...
    _ha_center_id_map = {}
    try:
        result = _vault_query("SELECT id, name__v FROM health_authority_division__v ORDER BY name__v ASC")
        if _is_success(result):
            for r in _all_pages(result):
                cid, name = r.get("id",""), r.get("name__v","")
                if cid and name:
...
def _resolve_country(cid): return _get_country_id_map().get(cid, cid)
def _resolve_type(tid):    return _get_app_type_id_map().get(tid, tid) if tid else "N/A"
def _resolve_format(fid):  return _get_dossier_format_id_map().get(fid, fid) if fid else "N/A"
def _resolve_region(rid):  return _get_region_id_map().get(rid, rid) if rid else "N/A"
def _resolve_ha(hid):      return _get_ha_id_map().get(hid, hid) if hid else "N/A"
...
_COUNTRY_ALIASES = {
    ...
    "britain": "united kingdom", "greatbritain": "united kingdom", "england": "united kingdom",
    # UAE
    ...
    # Korea
    "southkorea": "korea", "northkorea": "korea", "rok": "korea",
    # Others
    "czechrepublic": "czech",
    "drc": "congo",
    "holland": "netherlands",
...
def _market_abbrev(lead_market):
    ...
    return _MARKET_ABBREV.get(lead_market.lower().strip()) or lead_market.split()[0][:3].upper()
...
def _find_application_type(user_input):
    ...
    for name in _get_app_type_id_map().values():
        if name.lower() == lower: return name
    for name in _get_app_type_id_map().values():
        if f"({lower})" in name.lower(): return name
    for name in _get_app_type_id_map().values():
        if lower in name.lower(): return name
    return None

def _resolve_ha_id(user_input):
    lower = user_input.lower().strip()
    for hid, name in _get_ha_id_map().items():
        if name.lower() == lower: return hid, name
    for hid, name in _get_ha_id_map().items():
        if lower in name.lower(): return hid, name
    return None, None

def _name_exists(name):
    result = _vault_query(f"SELECT id FROM application__v WHERE name__v = '{name}'")
    if not _is_success(result): return False
    return len(result.get("data", [])) > 0

def _fn_get_lead_markets():
    try:
        result = _vault_query("SELECT lead_market__rim FROM application__v ORDER BY created_date__v DESC LIMIT 200")
        if not _is_success(result):
        ...
        return "The following Lead Markets already have existing applications in Vault: " + ", ".join(markets) + "."
    except Exception as e:
...
def _fn_get_product_families():
    ...
    for obj_name in candidates:
        try:
            result = _vault_query(f"SELECT id, name__v FROM {obj_name} ORDER BY name__v ASC")
            logger.info(f"product_family query [{obj_name}] → responseStatus={result.get('responseStatus')} "
                        f"errors={result.get('errors','none')}")
            if _is_success(result):
                values = [r.get("name__v","") for r in _all_pages(result) if r.get("name__v","")]
                if values:
                    logger.info(f"{obj_name} returned {len(values)} records: {values[:5]}")
                    return "Available Product Families in Vault: " + ", ".join(values) + "."
...
def _fn_get_dossier_format_for_market(lead_market):
    try:
        ...
        formats = list(dict.fromkeys(
            ...
            for r in result.get("data",[]) if r.get("dossier_format__v","")
        ))
        if not formats:
            return _no_vault_data(lead_market, "dossier format")
        if len(formats) == 1:
            return f"For {lead_market}, existing applications use the '{formats[0]}' Dossier Format."
        return f"For {lead_market}, existing applications have used: " + ", ".join(formats) + "."
    except Exception as e:
...
def _fn_get_dossier_formats():
    try:
        ...
        return "Available Dossier Formats in Vault: " + ", ".join(fmt_map.values()) + "."
    except Exception as e:
...
def _fn_get_application_types_for_market(lead_market):
    try:
        ...
        types = list(dict.fromkeys(
            ...
            for r in result.get("data",[]) if r.get("application_type__rim","")
        ))
        if not types:
            return _no_vault_data(lead_market, "application type")
        return f"For {lead_market}, existing applications have used: " + ", ".join(types) + "."
    except Exception as e:
...
def _fn_search_application_type(keyword):
    try:
        ...
        matches = [name for name in _get_app_type_id_map().values()
                   if f"({lower})" in name.lower() or lower in name.lower()]
        if not matches:
            return f"No Application Types found matching '{keyword}' in Vault."
        return f"Found {len(matches)} Application Type(s) matching '{keyword}': " + ", ".join(matches) + "."
    except Exception as e:
...
def _fn_validate_application_type_for_market(app_type, lead_market):
    try:
        ...
        country_id = _find_country_id(lead_market)
        if not country_id:
            return (f"'{matched_type}' is a valid Application Type in Vault. "
                    f"Use your general regulatory knowledge to confirm if it applies to {lead_market}.")
        result = _vault_query(
            f"SELECT application_type__rim FROM application__v "
            f"WHERE lead_market__rim = '{country_id}' ORDER BY created_date__v DESC LIMIT 10"
        )
        market_types = list(dict.fromkeys(
            _resolve_type(r.get("application_type__rim",""))
            for r in result.get("data",[]) if r.get("application_type__rim","")
        ))
        if not market_types:
        ...
        return (f"'{matched_type}' is valid in Vault but not used in existing {lead_market} applications. "
    ...
    try:
        ...
        for rid, name in region_map.items():
            if name.lower() == lower or lower in name.lower():
                ...
                break
        if not region_id:
            return f"Region '{region_name}' not found. Available: " + ", ".join(region_map.values()) + "."
        result = _vault_query(
            f"SELECT id, name__v FROM country__v WHERE region__v = '{region_id}' ORDER BY name__v ASC"
        )
        countries = [r.get("name__v","") for r in _all_pages(result) if r.get("name__v","")]
        if not countries:
...
def _fn_get_all_health_authorities():
    try:
        ...
        return f"There are {len(ha_map)} Health Authorities in Vault: " + ", ".join(ha_map.values()) + "."
    except Exception as e:
...
def _fn_get_health_authority_for_market(lead_market):
    try:
        ...
        authorities = list(dict.fromkeys(
            ...
            for r in result.get("data",[]) if r.get("health_authority__v","")
        ))
        if not authorities:
            return _no_vault_data(lead_market, "health authority")
        if len(authorities) == 1:
            return f"For {lead_market}, the Health Authority is: {authorities[0]}."
        return f"For {lead_market}, Health Authorities in Vault: " + ", ".join(authorities) + "."
    except Exception as e:
...
def _fn_get_health_authority_centers(ha_name):
    try:
        ...
        if not centers:
            return (f"VAULT_NO_RECORDS for {ha_name} centers. "
    ...
    try:
        ...
        for i in range(2, 10):
            ...
            if not _name_exists(candidate):
                return (f"Suggested Application Name: '{candidate}'. "
                        f"This name does not exist in Vault yet — safe to use.")
        return f"Suggested Application Name: '{base}'."
    except Exception as e:
...
def _fn_check_application_name(application_name):
    try:
        if _name_exists(application_name):
            return (f"'{application_name}' already exists in Vault — "
...
def lambda_handler(event, context):
    ...
    try:
        if function == "queryVault":
            ...
            logger.info(f"queryVault → queryType={query_type!r}, parameters={params_str!r}")
        ...
        response_text = "I encountered an error retrieving data from Vault. Please try again."
...
def _dispatch(query_type, p):
    if   query_type == "getLeadMarkets":                   return _fn_get_lead_markets()
    elif query_type == "getProductFamilies":               return _fn_get_product_families()
    elif query_type == "getDossierFormats":                return _fn_get_dossier_formats()
    elif query_type == "getDossierFormatForMarket":        return _fn_get_dossier_format_for_market(p("leadMarket"))
    elif query_type == "getApplicationTypesForMarket":     return _fn_get_application_types_for_market(p("leadMarket"))
    elif query_type == "searchApplicationType":            return _fn_search_application_type(p("searchKeyword"))
    elif query_type == "validateApplicationTypeForMarket": return _fn_validate_application_type_for_market(p("applicationType"), p("leadMarket"))
    elif query_type == "getRegions":                       return _fn_get_regions()
    elif query_type == "getRegionForMarket":               return _fn_get_region_for_market(p("leadMarket"))
    elif query_type == "getCountriesForRegion":            return _fn_get_countries_for_region(p("region"))
    elif query_type == "getAllHealthAuthorities":           return _fn_get_all_health_authorities()
    elif query_type == "getHealthAuthorityForMarket":      return _fn_get_health_authority_for_market(p("leadMarket"))
    elif query_type == "getHealthAuthorityCenters":        return _fn_get_health_authority_centers(p("haName"))
    elif query_type == "suggestApplicationName":           return _fn_suggest_application_name(p("productName"), p("applicationType"), p("leadMarket"))
    elif query_type == "checkApplicationNameExists":       return _fn_check_application_name(p("applicationName"))
    elif query_type == "getExistingApplications":          return _fn_get_existing_applications(p("leadMarket"))
    elif query_type == "getRecentApplications":            return _fn_get_recent_applications()
    else:
        logger.error(f"Unknown queryType: {query_type!r}")
        return (f"Unknown queryType '{query_type}'. Valid values: getLeadMarkets, getProductFamilies, "
